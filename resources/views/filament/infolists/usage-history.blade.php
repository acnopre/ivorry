        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-bold text-gray-700 dark:text-gray-300">Status</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-600">
                @foreach($usageHistory as $usage)
                @php
                    $statusBadge = match($usage->status) {
                        'signed'    => 'background-color:#dbeafe;color:#1d4ed8;',
                        'valid'     => 'background-color:#dcfce7;color:#15803d;',
                        'processed' => 'background-color:#f3e8ff;color:#7e22ce;',
                        default     => 'background-color:#f3f4f6;color:#6b7280;',
                    };
                @endphp
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td class="px-4 py-3 text-gray-900 dark:text-gray-100">
                        {{ $usage->availment_date?->format('M d, Y') ?? '—' }}
                    </td>
                    <td class="px-4 py-3 text-gray-900 dark:text-gray-100">
                        {{ $usage->member->first_name }} {{ $usage->member->last_name }}
                    </td>
                    ...
                    <td class="px-4 py-3 text-center text-gray-900 dark:text-gray-100 font-mono">
                        {{ $usage->quantity }}
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <span class="text-xs font-semibold px-2 py-1 rounded-full" style="{{ $statusBadge }}">
