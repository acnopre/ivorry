<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 border-l-4 border-primary-500 space-y-4">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 mb-4 dark:border-gray-700">
                    <div class="space-y-1">
                        <div class="font-bold text-2xl text-gray-900 dark:text-white">{{ $member->name }}</div>
                ...
                @php
                $procedures = \App\Models\Procedure::with(['units.unitType', 'units.unit', 'member'])
                ->where('member_id', $member->id)
                ->orderByDesc('availment_date') // Sort by most recent
                ->get();
                @endphp
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-600">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="px-4 py-2 text-gray-800 dark:text-gray-200">
                                        {{ optional($procedure->service)->name ?? 'N/A' }}
                                    </td>
                                    ...
                                    <td class="px-4 py-2">
                                        {{ $procedure->availment_date ? \Carbon\Carbon::parse($procedure->availment_date)->format('M d, Y') : '-' }}
                                    </td>
                                    <td class="px-4 py-2">
                                        @php
        ...
        <div x-data x-show="$wire.showProcedureModal" x-cloak x-trap.noscroll @click.away="$wire.showProcedureModal = false" x-on:keydown.escape.window="$wire.showProcedureModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" style="top:0; left:0; width:100vw; height:100vh;">
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow-2xl p-6 w-full max-w-lg relative">
                <button @click="$wire.showProcedureModal = false" class="absolute top-3 right-3 z-50 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 focus:outline-none focus:ring focus:ring-primary-300 rounded-full p-1 transition">
