<x-filament-panels::page>
    ...
    <div x-data="{ show: false, status: 'Preparing document...', progress: 0 }" 
         @start-printing.window="show = true; status = 'Preparing document...'; progress = 0; console.log('Modal opened')"
         @update-progress.window="status = $event.detail.status; progress = $event.detail.progress; console.log('Progress:', progress, status)"
         @close-printing.window="show = false; status = 'Preparing document...'; progress = 0; console.log('Modal closed')"
         x-show="show" 
         x-cloak
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 shadow-xl">
            <div class="text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Printing ADC</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4" x-text="status"></p>
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                    <div class="bg-primary-600 h-2.5 rounded-full transition-all duration-300" :style="`width: ${progress}%`"></div>
                </div>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-2" x-text="`${progress}% complete`"></p>
            </div>
        </div>
    </div>
</x-filament-panels::page>
