...
class DentistSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Create the "Dentist" role and assign permissions
        $role = Role::firstOrCreate(['name' => 'Dentist']);
        $permissions = ['dentist.view', 'dentist.manage', 'chatbot.use']; // example permissions
        $role->syncPermissions(Permission::whereIn('name', $permissions)->get());

        // Loop to create dentists
        for ($i = 1; $i <= 15; $i++) { // total 15 dentists (3 per clinic × 5 clinics)
            ...
            // Create user account for the dentist
            $user = User::create([
                'name'     => $fullName,
            ...
            DB::table('dentists')->insert([
                ...
                'is_owner'            => $i % 3 === 1 ? 1 : 0, // every 1st in group of 3 is owner
                'created_at'          => now(),
                'updated_at'          => now(),
            ]);
        }

        $this->command->info('✅ Dentists and user accounts seeded successfully!');
    }
}
